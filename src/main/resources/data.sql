...
INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Научная конференция', 'Конференция по искусственному интеллекту', '2023-12-15 10:00:00', 'Москва, Университет ИТ', 150, 1, 'SCIENCE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Научная конференция');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Футбольный матч', 'Финал чемпионата города', '2023-11-20 18:30:00', 'Казань, Стадион «Ак Барс»', 1000, 2, 'SPORT', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Футбольный матч');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Выставка современного искусства', 'Работы молодых художников', '2024-01-10 12:00:00', 'Санкт-Петербург, Эрмитаж', 200, 3, 'CULTURE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
...
INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Лекция по биохимии', 'Новые открытия в медицине', '2023-12-01 14:00:00', 'Новосибирск, НГУ', 30, 4, 'SCIENCE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Лекция по биохимии');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Хакатон по разработке', 'Соревнования для программистов', '2024-02-05 09:00:00', 'Иннополис', 300, 4, 'SCIENCE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Хакатон по разработке');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Беговой марафон', 'Городской марафон на 10 км', '2023-11-25 08:00:00', 'Сочи, Набережная', 1000, 2, 'SPORT', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Беговой марафон');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Книжная ярмарка', 'Продажа редких книг', '2024-03-15 11:00:00', 'Екатеринбург, Дом Книги', 140, 3, 'CULTURE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Книжная ярмарка');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Семинар по робототехнике', 'Практические мастер-классы', '2023-12-10 16:30:00', 'Томск, ТПУ', 50, 3, 'SCIENCE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Семинар по робототехнике');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Баскетбольный турнир', 'Школьные команды', '2024-04-20 17:00:00', 'Краснодар, Спорткомплекс', 200, 1, 'SPORT', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Баскетбольный турнир');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Опера "Евгений Онегин"', 'Классическая постановка', '2024-05-01 19:00:00', 'Москва, Большой театр', 500, 2, 'CULTURE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Опера "Евгений Онегин"');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Лекция по астрофизике', 'Исследование черных дыр', '2024-06-12 15:45:00', 'Самара, СГАУ', 60, 3, 'SCIENCE', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
    WHERE NOT EXISTS (SELECT 1 FROM event WHERE title = 'Лекция по астрофизике');

INSERT INTO event (title, description, date, location, participant_limit, organizer_id, category, image_url)
SELECT 'Велокросс', 'Гонки по пересеченной местности', '2024-07-07 10:00:00', 'Уфа, Парк Победы', 200, 1, 'SPORT', 'http://localhost:9000/event-horizon/events/images/fe39ff10-6fbf-44ce-9023-24d692f81e49'
